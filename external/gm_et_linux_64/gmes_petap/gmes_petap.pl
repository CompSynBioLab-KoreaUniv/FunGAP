...
# ---
# default values are overwritten by values from config file
# ---
...
sub TrainingDataReport
{
	...
	RunCom( "$bin/probuild --seq data/training.fna --stat info/training.general --allow_x  --GC_PRECISION 0 ");
}
# ------------------------------------------------
sub SplitFasta
{
...
sub CommitTrainingData
{
	...
	# this function splits sequence for training and for prediction steps
	...
	# ET
	RunCom( "$bin/rescale_gff.pl  --in data/et.gff  --trace info/training.trace  --out data/et_training.gff" ) if $cfg->{'Parameters'}->{'ET'};
	
	# evidence
	RunCom( "$bin/rescale_gff.pl  --in data/evidence.gff  --trace info/training.trace  --out data/evidence_training.gff") if $cfg->{'Parameters'}->{'evidence'};
}
# ------------------------------------------------
sub DataReport
{
	...
	RunCom( "$bin/probuild  --seq data/dna.fna  --allow_x  --stat info/dna.general" );
...
#		RunCom("$bin/reformat_fasta.pl  --up  --out data/dna.fna  --label _dna  --trace info/dna.trace  --in $name  $str" );
	}
...
sub ParseCMD
{
	...
	# this option replaces defaults to fungal specific
...
{
	# switch algorithm sections: ON or OFF
	...
	# switch algorithm sections: ON or OFF
	$cfg->{'Parameters'}->{'training_only'}   = ''; # if true, run only training step
	$cfg->{'Parameters'}->{'prediction_only'} = ''; # if true, run only prediction step
	...
	$cfg->{'Parameters'}->{'soft_mask'}  =       0; # false, if true than mask softmasked string longer than specified by "soft_mask" parameter 
	...
	$cfg->{'Parameters'}->{'max_intergenic'} = 0;  # 0 - use default method for parameter initiation
	$cfg->{'Parameters'}->{'max_intron'}     = 0;  # 0 - use default method for parameter initiation
	
	$cfg->{'Parameters'}->{'min_gene_prediction'} = 300;  # minimum gene length to predict

	$cfg->{'Parameters'}->{'ini_mod'}    = ''; # use this mofrl file for initiation of iterations
	$cfg->{'Parameters'}->{'usr_cfg'}    = ''; # in addtion to default behaviar, read parameters from this file
...
Customizing parameters:
  ...
  --max_intergenic      [number]; default $cfg->{'intergenic_DUR'}->{'max'}, maximum length of intergenic regions
  --min_gene_prediction [number]; default $cfg->{'Parameters'}->{'min_gene_prediction'} ($cfg->{'Fungi'}->{'min_gene_prediction'} fungi) minimum allowed gene length in prediction step
