...
or UniProt database"
    ...
    for est_file in est_files:
        ...
        snap_hmm_file_run1 = train_snap(
            root_dir, all_gff_file_run1, '1', est_prefix
        )
        ...
        snap_hmm_file_run2 = train_snap(
            root_dir, all_gff_file_run2, '2', est_prefix
        )
...
def run_gmes(
    masked_assembly, num_cores, root_dir
):
    ...
    if not glob(output_gmes):
        os.chdir(gmes_path)
        command = (
            'gmes_petap.pl --ES --fungus --cores %s --sequence %s '
            '--soft_mask 1 > %s' % (
                num_cores, masked_assembly, log_file
            )
        )
...
def run_maker_batch(
    ...
    command = 'maker -fix_nucleotides > %s 2>&1' % (maker_log)
    logger_txt.debug('[Run] %s' % (command))
    os.system(command)
...
def run_maker_trained(
    ...
    all_gff_file, version, prefix, eukgmhmmfile=None
):
...
def train_snap(root_dir, all_gff_file, version, prefix):
    maker_run_dir = os.path.join(
        root_dir, software, prefix, 'maker_run%s' % (version)
    )
    ...
    if not os.path.exists(snap_hmm_file):
        ...
        command1 = 'maker2zff -n %s' % (all_gff_file)
        ...
        command2 = 'fathom -categorize 1000 genome.ann genome.dna'
        logger_txt.debug('[Run] %s' % (command2))
        os.system(command2)

        # Export the genes
        command3 = 'fathom -export 1000 -plus uni.ann uni.dna'
        ...
        command4 = 'forge ../export.ann ../export.dna'
        logger_txt.debug('[Run] %s' % (command4))
        os.system(command4)

        # Generate the new HMM
        os.chdir('..')
        command5 = 'hmm-assembler.pl snap_hmm_v%s parameters > \
snap_hmm_v%s.hmm' % (
            version, version
        )
