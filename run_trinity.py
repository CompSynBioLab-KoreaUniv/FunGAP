...
def main(argv):
    ...
    # Run functions :)
    run_trinity(bam_files, output_dir, log_dir, project_name, num_cores)
...
def run_trinity(bam_files, output_dir, log_dir, project_name, num_cores):
    output_base = os.path.basename(output_dir)
    ...
    for bam_file in bam_files:
        ...
        if not os.path.exists(new_output):
            ...
            command = (
                'Trinity --genome_guided_bam %s --genome_guided_max_intron %s '
                '--max_memory %s --CPU %s --output %s --jaccard_clip > '
                '%s 2>&1' % (
                    bam_file, max_intron, max_memory, num_cores, outdir,
                    log_file
                )
            )
